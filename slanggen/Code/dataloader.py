...
class Dummy_ZH_Dataset(ConvDataset):
    
    def load_data(self, data_path):
        data_ZH = np.load(data_path, allow_pickle=True)
        vocab_ZH = set([w.word for w in data_ZH])
        return data_ZH, vocab_ZH
...
class OSD_Dataset(SlangDataset):
    ...
    def load_data(self, slang_path, conv_dataset):
        ...
        data_OSD = [process_def(d) for d in data_OSD_raw if d[0] in conv_dataset.vocab]
        
        return data_OSD

class ZH_Dataset(SlangDataset):
    
    def __init__(self, slang_path, conv_dataset):
        super().__init__(slang_path, conv_dataset)
        
    def load_data(self, slang_path, conv_dataset):
        data_ZH_raw = np.load(slang_path, allow_pickle=True)
        ...
        data_ZH = [process_def(d) for d in data_ZH_raw if d[0] in conv_dataset.vocab]
